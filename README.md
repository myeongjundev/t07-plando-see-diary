...
<picture>
  ...
  <img alt="이메일과 비밀번호로 접속하는 플랜두씨 다이어리 로그인 화면" src="docs/screenshots/login-hero.png">
</picture>
...
| 현재 상태 | T07 인증·기록 기능 운영 중 · 실제 5일 관찰 완료 · 별도 설정 페이지 최종 검증 완료 |
...
- 기간, 우선순위, 예상 시간, 성공 기준을 포함한 계획 생성
- 예상 시간 수정과 변경 전 계획 이력 보존
...
- 상단 설정 메뉴와 보호된 `/settings`에서 프로필·내보내기·보안 기능 관리
- 회원가입 직후에도 기존 사용자와 동일한 로그인 경로를 거쳐 세션 발급
- 로그인하지 않은 사용자의 앱 화면 접근 차단
- 모든 조회·수정·삭제를 현재 계정 소유 데이터로 제한
- 로그아웃 시 해당 세션 폐기, 비밀번호 변경 시 기존 모든 세션 폐기 후 현재 기기에 새 세션 발급
...
같은 계열 전체를 폐기합니다. 프런트엔드는 탭 내부에서는 요청 큐, 탭 사이에서는 Web Locks로
갱신을 직렬화합니다. 잠금을 얻은 뒤 원 요청을 다시 보내 이미 갱신됐으면 추가 Refresh를
생략합니다. Web Locks가 없을 때는 BroadcastChannel로 진행 상태를 공유하지만, 이 대체
경로는 잠금과 같은 수준의 동시 실행 방지를 보장하지 않습니다.
...
막습니다. 그래서 변경 전 백업 → 계정·nullable 소유권 추가 → 검토한 기존 행 claim → 미소유 행
...
직접 구현한 인증 흐름 위에 검증된 라이브러리의 암호 기능을 사용했습니다. 해싱이나
...
2026-09-11 관찰 증거를 보존한 뒤 실행한 로컬 검증 결과입니다. PostgreSQL 결과는 별도로
표시한 이전 검증 기록이며, 아래 수치는 같은 환경에서 한 번에 실행한 결과가 아닙니다.

| 검증 | 결과 |
| --- | --- |
| Backend pytest | **319 passed, 3 skipped** |
| Frontend Vitest | **76 passed** |
| TypeScript + Vite production build | **passed** |
| PostgreSQL 전용 검사 | 2026-09-05 별도 Neon 테스트 브랜치에서 통과 |
| 비밀값 패턴 감사 | worktree · build · Git objects **0 findings** |

로컬의 3개 skip은 PostgreSQL 전용 검사이며 2026-09-05 별도 Neon 테스트 브랜치에서
통과했습니다. 실제 관찰은 2026-09-07~11의 서로 다른 Seoul 날짜 5일에 완료했고,
운영 응답과 내보내기를 대조한 증거로 관찰 검사까지 통과했습니다.
...
| 문서 | 내용 |
| --- | --- |
| [포트폴리오 소개글](docs/PORTFOLIO.md) | 한 줄 소개, 프로젝트 설명, 주요 구현과 검증 근거 |
