...
아래 2026-09-03 본문은 초기 설계 기록이다. **새 벤치마크 서비스를 운영 DB에 연결하는
절차와 마지막의 두 배포 claim 절차는 더 이상 실행하지 않는다.** 현재 마이그레이션은
T07 계정과 소유권을 실제로 변경한다. 배포의 기준 절차는
`../process/T07-HANDOFF-2026-09-04.md` 4절과 `../../deploy/start.sh`다.

1. 기존 T06 서비스에서 claim 배포를 완료하고 주인 없는 계획 0건과 로그인 화면을 확인한다.
   현재 start.sh는 계정 revision까지 업그레이드 → claim → head까지 업그레이드 순서다.
2. 같은 Render 서비스의 환경변수에서 `BOOT_TASK`를 `bench_password_hashing`,
   `BOOT_TASK_ARGS`를 `--repeats 3`으로 변경하고 배포한다. 벤치마크는 임시 합성값만 사용하며
   10초 후 백그라운드로 시작한다. 이 작업은 실제 사용 1일차 전에 끝낸다.
3. 로그의 BEGIN부터 END까지를 `00-hash-bench-render.md`에 기록한다. 단독 p50/p95,
   동시 2·4건 벽시계, peak RSS와 실행 환경을 포함한다. 완료되지 않은 측정을 통과로 적지 않는다.
4. `BOOT_TASK=none`과 빈 `BOOT_TASK_ARGS`로 되돌려 재배포한다. 저장소의 render.yaml도
   비활성 상태로 맞춘다. **공개 저장소 URL 연결은 Blueprint 없이 자동 배포되지 않으므로
   환경변수 저장과 수동 배포 결과를 각각 확인한다.**

## 초기 설계 기록 — 현재 실행 지침 아님
