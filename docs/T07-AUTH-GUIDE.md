...
2026-09-11 운영 배포·이관·Render 해싱 벤치마크와 실제 5일 관찰을 확인했다.
아래 인증 요청·응답은 합성 계정으로 실행한 로컬 증거이며 실제 일기 기록이 아니다.
실사용 결과는 [5일 관찰 증거](T07-EVIDENCE/observation-five-days.md)에 분리했다.
...
다음은 2026-09-05 로컬 가상환경에서 확인한 버전이다. 운영 이미지는 같은
`backend/pyproject.toml`로 빌드되었고, 운영 실행 환경은 해싱 증거의 Linux·Python
버전으로 별도 남겼다. 범위 지정 의존성은 재빌드 시 달라질 수 있다.
...
[Render 운영 측정](T07-EVIDENCE/00-hash-bench-render.md)에서 현재 Argon2id 기본값은 검증
p50 **108ms**, p95 **197ms**로 500ms 예산을 모두 만족했다. 동시 4건 벽시계는
1,009ms, peak RSS는 146MiB였다. 따라서 메모리 하드 성질을 버리지 않고 현재
Argon2id 매개변수를 유지했다.
...
2026-09-05 운영 이관을 완료했다. 이관 전 Neon 백업을 만들고 계획 3건을 고정 계정으로
claim한 뒤 주인 없는 계획이 0건임을 확인했으며, 이후 `plans.user_id NOT NULL` revision까지
적용했다. 배포·백업 식별자와 건수는 [제출 기록](T07-SUBMISSION.md)에 남겼다.
...
내가 직접 판단한 일: 실제 기록을 꾸미지 않고 5일을 기다렸으며, 2일차 결과를 보고 이후
작업을 10분 단위로 나누기로 판단했다.

AI 제안을 따르지 않은 일: 별도로 거절한 제안은 없었다. 다만 AI의 결과를 그대로 채택하지
않고 운영 기록과 손계산, 테스트 결과를 직접 대조한 뒤 결정했다.
...
관찰은 [고정 프로토콜](T07-STUDY-PROTOCOL.md)에 따라 2026-09-07~11의 실제 달력 5일을
완료했다. 규칙 변경은 2일차 기록 뒤·3일차 기록 앞에 한 번 남겼고, 운영 응답·내보내기와
실제 합계를 [관찰 증거](T07-EVIDENCE/observation-five-days.md)에서 검산했다.
