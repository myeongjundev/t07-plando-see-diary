...
상태: **구현·운영 배포·Argon2id 측정·실제 5일 관찰 완료(2026-09-11).**
...
앱은 공개 인증 화면과 로그인 뒤의 보호 화면을 주소로 나눈다. C03·C97은 화면을 숨기는
것이 아니라, 보호 경로가 세션 관문을 반드시 지나도록 요구한다.

- `react-router-dom` 도입
- `/login`, `/signup` — 로그인하지 않아도 열린다 (C03: 심사자가 계정 없이 여기까지는 봄)
- `/app` — 자료 화면. 로그인 안 했으면 **`/login`으로 보낸다** (C97)
- `/settings` — 프로필·내보내기·비밀번호 변경·계정 삭제. `/app`과 같은 관문을 쓴다
- Flask에 SPA 폴백 라우트 추가: `/login`·`/signup`·`/app`·`/settings` 직접 접근 시 `index.html`
...
계정 화면(`/settings`): 프로필 · 내보내기 · 비밀번호 변경 · **계정 삭제**(지워진다는 안내
문구 포함, C134). `/app`은 자주 쓰는 Plan·Do·See 흐름에서 끝난다.
...
- **비밀번호 재설정 없음** — 잊으면 **계정이 끝난다.** 자료도 함께 끝난다: 되찾을 길이
  없고, 이 앱에는 관리자 화면도 없다. 편의 문제로 보이지만 실제로는 자료 상실 경로다.
  붙이려면 메일 발송이 전제이고(아래), 급하게 붙이면 재설정 링크가 **비밀번호보다 약한
  두 번째 열쇠**가 된다 — 유효 기간, 일회성, 재설정 시 전 세션 폐기가 전부 필요하다.
  기업용 가입 템플릿이 거의 예외 없이 들고 있는 항목이고, 우리에게 없는 것이다.
- **두 번째 인증 수단 없음** — 비밀번호 하나가 새면 그걸로 끝이다.
- **이메일 소유 확인 없음** — 남의 주소로 가입할 수 있고, 그 주소의 주인은 자기 이름으로
  된 계정이 생긴 것을 모른다. 나중에 진짜 주인이 가입하려 하면 「이미 가입된 이메일」을
  만나고, 되찾을 방법이 없다(위 항목과 같은 막다른 길). 이것도 **메일 발송이 전제다.**

  **왜 안 만들었나**: 이 배포에는 메일을 보낼 수단이 없다. Render Free에 발송 기능이
  없고, 외부 발송 서비스를 붙이는 것은 새 비밀값과 새 실패 경로를 하나씩 더 만드는
  일이며, 과제 기준 어느 것도 요구하지 않는다. **못 하는 것이 아니라 범위 밖으로 둔
  것**이고, 그래서 여기 적는다. 이 하나가 있었으면 위의 「가입 화면이 계정 존재를 알린다」
  까지 같이 닫혔다 — 가입이 언제나 같은 응답을 주고 갈라짐을 메일로 옮길 수 있으니까.
...
- **가입에는 속도 제한이 없다** — 잠금 장치(`throttle`)는 로그인에만 걸려 있다. 가입은
  그냥 통과라, 공개 주소에서 누구든 계정을 무제한으로 만들 수 있다. 심사하는 사람이
  자기 주소로 한 번 가입하는 것은 이 앱이 기대하는 동작이지만(C94), 같은 문이 자동화된
  대량 생성에도 열려 있다. **왜 안 막았나**: 로그인 잠금은 「어느 계정이 공격받는가」라는
  자연스러운 키가 있어 `login_attempts` 하나로 끝났는데, 가입에는 그 키가 없다. IP만으로
  묶으면 공용 회선 뒤의 정상 사용자를 같이 막고, 이메일로 묶는 것은 아직 계정이 없으니
  의미가 없다. 제대로 하려면 확인 메일이나 CAPTCHA인데 **둘 다 이 배포에 없는 전제**이고,
  과제 기준 어느 것도 요구하지 않는다. 남는 피해는 무게가 다르다 — 로그인 뚫기는 자료
  유출이고, 가입 남용은 빈 계정 행이 쌓이는 것이다. 그래서 범위 밖으로 두고 여기 적는다.
...
- **비밀번호 구성 규칙을 넣었다** — 8자 이상·영문·숫자를 가입에서 강제한다(`accounts.py`).
  NIST SP 800-63B는 구성 규칙을 **권하지 않는다**: 사람을 `Password1!` 같은 예측 가능한
  치환으로 몰기 때문이다. 제품 요구로 넣었고, 대문자·특수문자는 강제하지 않아 규칙이
  모양까지 지시하지는 않게 했다. 온라인 추측에 실제로 값을 물리는 것은 여전히 6절의
  잠금이지 이 규칙이 아니다.
- **강도 표시가 볼 수 없는 것** — `passwordStrength.ts`는 길이·종류·반복·연속·자판 한 줄과
  짧은 흔한 단어 목록만 본다. **유출 목록도 개인 정보도 보지 않는다.** 그래서 자기 이름과
  올해를 섞은 `Myeongjun2026!` 같은 값이 「강함」으로 표시된다 — 실제로는 표적 공격에
  가장 먼저 시도되는 모양이다. 제대로 닫으려면 HIBP 같은 유출 목록 대조가 필요하고,
  그건 외부 호출이다.
- **`IP_HASH_SECRET`을 돌리면 로그인 제한이 초기화된다** — IP는 원문 대신 이 키로 HMAC해
  저장하므로, 키가 바뀌면 과거 시도와 새 시도가 이어지지 않는다. 즉 **키 회전이 곧 모든
  잠금 해제**다. 운영 절차에 적어 두는 것 말고 코드로 막을 방법이 없다.
- **거절 경로도 크래시할 수 있다** — `csrf_matches`가 비ASCII 헤더에서 `TypeError`를 내
  403이어야 할 자리에 500을 주고 있었다(증거 스크립트가 찾았고 고쳤다). 무해해 보이지만
  **그 하나만 다른 거절과 구별된다.** 같은 부류가 더 없다고 말할 근거는 없고, 있다면
  같은 방식으로 드러날 것이다.
...
| 2 | Argon2id 비밀번호 서비스 + Render 동시성·RSS 측정 | 같은 비밀번호 두 해시가 다름, p50 108ms·p95 197ms |
...
21. 설명서 여섯 항목 · 확인 4줄 — **끝남**. 판단 3줄 중 본인 작성 두 줄만 사용자가 확정

5일 기록은 **지정한 관찰 plan의 다이어리 사용 기록**으로 2026-09-07~11에 완료했다.
2일차 실행 뒤·3일차 실행 전에 규칙 변경을 정확히 한 번 남겼고, 관찰 증거를 보존할 때까지
배포를 멈춰 기록 순서와 기준을 지켰다.
...
| 비밀번호 보관 | **확정·측정 완료** — Argon2id / `argon2-cffi`, 19,456KiB·t=2·p=1. Render p50 108ms·p95 197ms, 동시 4건 peak RSS 146MiB |
